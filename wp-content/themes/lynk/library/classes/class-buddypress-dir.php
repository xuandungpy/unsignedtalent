...
class jvbpd_buddypress {
	...
	function adminbar_notification() {
		return @$GLOBALS[ 'bb_adminbar_notifications' ];
	}
	...
	public function bp_get_message_thread_total_and_unread_count_number( $thread_id = false ) {
		...
		return sprintf(
			...
			sprintf( _n( '%d unread', '%d unread', $unread, 'jvfrmtd' ), number_format_i18n( $unread ) )
		);
	}
...
/*** Adding listing menus **/
	...
	public function bp_template_content() {
		get_template_part( 'library/classes/dir/' . BuddyPress()->current_component . '/' . BuddyPress()->current_action );
	}
...
/*** header options **/
	...
	public function module_selector( $section='' ) {
		...
		if( in_array( $section, $targets ) ) {
			?>
			<div>
				<span><?php esc_html_e( "Loop module style", 'jvfrmtd' ); ?> : </span>
				<select name="jvbpd_ts[<?php echo esc_attr( $section ); ?>loop_module]">
					<?php
					foreach(
						Array(
							'' => esc_html__( "Defalut", 'jvfrmtd' ),
							'small-box' => esc_html__( "Smail Box", 'jvfrmtd' ),
							'bg-box' => esc_html__( "BG-Box", 'jvfrmtd' )
	...
	public function header_parallax( $section='' ) {
		...
		if( in_array( $section, $targets ) ) {
			?>
			<div>
				<span><?php esc_html_e( "Background parallax", 'jvfrmtd' ); ?> : </span>
	...
	public function navigation_type( $section='' ) {
		...
		if( in_array( $section, $targets ) ) {
			?>
			<div>
				<span><?php esc_html_e( "Navigation Type", 'jvfrmtd' ); ?> : </span>
				<select name="jvbpd_ts[<?php echo esc_attr( $section ); ?>navi_type]">
					<?php
					foreach(
						Array(
							'' => esc_html__( "Type 1", 'jvfrmtd' ),
							'bp-header-type-2' => esc_html__( "Type 2", 'jvfrmtd' ),
						) as $strHeaderPallax => $strOptionLabel
					) {
						printf(
							'<option value="%1$s"%3$s>%2$s</option>',
	...
	public function getNotifyArgs() {
		...
		if( function_exists( 'buddypress' ) && function_exists( 'bp_notifications_get_registered_components' ) ) {
			...
			foreach ( $arrResults[ 'components' ] as $component ) {
				$arrQuery[] = $GLOBALS[ 'wpdb' ]->prepare( '%s', $component );
			}
