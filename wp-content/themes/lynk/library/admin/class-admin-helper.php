...
if( class_exists( 'Lynk_Core' ) && ! property_exists( 'Lynk_Core', 'prefix' ) ) {
	...
	wp_die(
		esc_html__( "The old version of Core plugin has been deactivated due to compatibility. Please update Core plugin to the newest version.", 'jvfrmtd' ) .
		sprintf( '<p><a href="%1$s" target="_self" title="%2$s">%2$s</a></p>', admin_url( 'plugins.php' ), esc_html__( "Go to plugin update", 'jvfrmtd' ) )
	);
}
...
if( class_exists( 'Jvbpd_Core_AdminHelper' ) ) {
	class jvbpd_admin_helper extends Jvbpd_Core_AdminHelper {
		...
		public function createHelperMenu() {
			...
			$this->addSub( Array(
				'slug' => 'status',
				'name' => esc_html__( "Status", 'jvfrmtd' ),
				'func' => array( $this, 'helper_staus' ),
			) );

			$this->addSub( Array(
				'slug' => 'plugins',
				'name' => esc_html__( "Plugins", 'jvfrmtd' ),
			...
			if( class_exists( 'jvbpd_theme_settings' ) ) {
				...
				$this->addSub( Array(
					'slug' => 'settings',
					'name' => esc_html__( "Theme Settings", 'jvfrmtd' ),
		...
		public function tgmpa_return_link( $link='' ) {
			if(
				...
				$link = sprintf(
					'<a href="%1$s">%2$s</a>',
					esc_url( $url ),
					esc_html__( 'Return Javo Core Plugin Page', 'jvfrmtd' )
				);
			}
			return $link;
		}
